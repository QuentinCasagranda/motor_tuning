...
We have two control loops to tune, the velocity control loop and the position control loop. The velocity control loop is managed by the EL7342 module, it takes the velocity command from the NC task. The position control loop is managed by the NC task, it takes the position command from the user.
We don't use the NC task to manage the velocity of the motor because the EL7342 has a much faster cycle time than the NC task that is too slow to manage the velocity control loop efficiently.
 
The tricky part of this control loop is the interaction between the NC task and the EL7342. The NC task is sending velocity commands to the EL7342 using a 16bits signed integer, this means that the NC task and the EL7342 need to have the same velocity reference for the FSR of this integer. 
The thing is that they aren't using the same unit in their configuration, the NC task is using mm/s while the EL7342 is using RPM but you need to have the exact value of the FSR in both configuration to have a correct behavior of your motor.
...
Configuration of the NC task links
==================================
...
Configuration of the NC Task and the EL7342 parameters
======================================================
Now we will need to configurate our NC Task and our EL7342 module parameters. 
...
For this part you will need the first two sheet of the document (NC Configuration and EL7342 Configuration).
In the NC Configuration sheet, enter your motor's characteristics in the first table. 

Then just Take the calculated values and copy them in The NC task and EL7342 CoE Online configurations of your project. 



Create a YT chart
=================
To visualize the changes of the motor's velocity and position during the tuning procedure, we will be using a YT chart.
To create a YT chart, open a new twinCAT project and select measurement wizard.
...
Then choose a classic chart and for the data select your plc and the nc task of your project.
...
Now you can take the variables that are interesting for you :
...
You can display the velocity, position and position error on three different axes to have a better veiw of the motor's behavior during the tuning procedure.
...
.. warning:: 
   ...
   REMINDER :  The tuning can be affected by the cycle time associated to the NC. It is recommended  to set this task to 1 ms before any tuning,  for better results and less oscillations.  Go to the "Motor and software configuration" chapter II part 1 to set the correct cycle time.

Configurating the Mode
----------------------

To have our EL7342 working as a velocity control loop, we have to setup some parameters in the Process Data and the CoE Online. Left click on your Terminal EL7342, and go in the Process Data window.
...
The Ziegler-Nichols method's first step is to find the ultimate Kp gain to which the velocity of the motor is no longer stable, and start oscillating. 
Increase the Kp factor, **to update the changes, active the "Auto Update" option or just update list in the CoE Online tab.**
...
Once the changes are updated, go in the NC-online panel and activate the controller:
...
It is not mandatory to use PID controller, you can choose to use a PD, PI or P controller, for eaxample on a M111-1DG, the PID controller is not efficient and a PD controller is enough to have a good behavior of the motor.

You can change the type of controller you want to use in the spreadsheet and see wich gives you the best response.
...
The controller of the NC task is configurated in the Ctrl part of our Axis.
...
Caracterization of the motor tunning
====================================
...
Send three to four velocity step sequences of 4 cycle with a velocity starting from the nominal velocity of your motor and divide it by ten for each sequence. 
...
For the position precision caracterization, you can use the online panel. You can mannualy send a sequence with a target position positive then 0 then a target position negative. Alike the velocity step sequence, you can repeat this sequence with different position values, starting from a potition far from the homing position and divide it by ten for each sequence.
...
.. warning:: 
   ...
   TIP :  To control the motor mannually with the online panel, it is important to set the override to 100% to be able to move the motor
